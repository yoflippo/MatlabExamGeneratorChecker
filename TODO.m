C:\Users\yoflippo\Dropbox\BT\00_ALG_BT\00-NEWCURR\
Biostatica_Auto_Matlab\clean_source\assignments\week2\deelopdracht_2\opdracht_7

Het maken van functies met meer dan 3 inputs outputs

%% Goed begrijpen van verschillende functie test


%% Automatisch herkennen en testen van functies met meerder inputs en outputs
% Het verzamelen van verschillende vragen + uitwerkingen die een scala van
% I/O representeren

% Checken van aantal Inputs
% - tel kommas
% - feval gebruiken

% Checken van aantal Outputs  op basis van solution file
% - tel kommas
% - feval gebruiken