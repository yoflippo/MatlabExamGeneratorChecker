function CreateBonusAssignments(con,BonusWeeks)
...
InitAll
dbstop if error
for nW = length(BonusWeeks)
    
    % Copy assignment zero
    apAss0 = fullfile(con.Assignments,['week' num2str(BonusWeeks(nW))],'deelopdracht_0');
    mkdirIf(apAss0);
    copyfiles(fullfile(con.BASEFOLDER,con.DIRHEADER,'deelopdracht_0'),apAss0);
    
    debugOutput(DEBUGOUTPUT,['Generating assignents for WEEK: ' num2str(BonusWeeks(nW))]);
    weekNr = num2str(BonusWeeks(nW));
