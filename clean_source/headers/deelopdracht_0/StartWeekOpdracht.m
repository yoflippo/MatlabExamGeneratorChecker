...
if contains(pwd,'.zip')
    errtxt = [newline newline '  Je hebt het door ons aangeleverde zip-bestand niet uitgepakt!' newline 'Unzip (uitpakken) het zip-bestand NU!'];
    error(errtxt);
end
if ~any(contains({dirs.name},'deelopdracht_1')) || ~any(contains({dirs.name},'deelopdracht_2')) || ~any(contains({dirs.name},'AfrondenWeekOpdracht.m'))
    error('Er missen folders die nodig zijn om een zinvol bestand aan te leveren. Kijk eens naar wat wij hebben aangeleverd en herstel de folders en structuren.');
end
